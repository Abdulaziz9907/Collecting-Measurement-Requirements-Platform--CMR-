...
export default function Standards() {
  ...
  const LocalTheme = () => (
    <style>{`
      ...
      @media (max-width:576px){
        ...
        .m-toolbar{ display:grid; grid-template-columns:repeat(3,1fr); gap:6px; }
      ...
      .mobile-title{ font-weight:700; color:var(--text); font-size:.95rem; line-height:1.3; }
    ...
    <Popover id="pop-template-help" dir="rtl">
      ...
      <Popover.Body>
        ...
        <ul className="mb-0 ps-3">
          ...
          <li>صيغة رقم المعيار: <code>XX.XX.XX</code>.</li>
          ...
          <li>احفظ الملف بصيغة Excel ثم استخدم زر «استيراد Excel».</li>
        </ul>
      </Popover.Body>
    </Popover>
  );
    ...
    try {
      ...
      for (const r of rows) {
        ...
        try {
          ...
          if (okRes) { ok++; batchSeen.add(stdNumNorm); existingNumbers.add(stdNumNorm); } else { fail++; }
        } catch { fail++; }
      }
      ...
      setBanner({ type: 'danger', text: 'تعذر قراءة الملف. تحقق من الأعمدة وجرب القالب.' });
    } finally {
      setImporting(false);
  ...
  const selectAllResults = () => { const all = sortedData.map(r => r?.standard_id).filter(Boolean); setSelectedIds(new Set(all)); };
  ...
  const performBulkDelete = async () => {
    ...
    if (batchOk) { setBanner({ type: 'success', text: `تم حذف ${ids.length} معيارًا بنجاح.` }); closeBulkDelete(); clearSelection(); await refreshData(); return; }
    const res = await deleteManyFallback(ids);
    closeBulkDelete(); clearSelection(); await refreshData();
    ...
    return (
      <div className="mobile-card" key={id}>
        <div className="mobile-card-header">
          ...
          <span className={`badge bg-${getStatusClass(item.status)}`} style={{ whiteSpace:'nowrap' }}>{item.status}</span>
        </div>
                            ...
                            <div className="controls-inline">
                              <Dropdown align="end">
                                ...
                                <Dropdown.Menu renderOnMount>
                                  ...
                                  <Dropdown.Item onClick={() => {setSortKey('standard_name'); setSortDir('asc');}} active={sortKey==='standard_name' && sortDir==='asc'}>الاسم (أ-ي)</Dropdown.Item>
                                  <Dropdown.Item onClick={() => {setSortKey('standard_name'); setSortDir('desc');}} active={sortKey==='standard_name' && sortDir==='desc'}>الاسم (ي-أ)</Dropdown.Item>
                                  <Dropdown.Item onClick={() => {setSortKey('standard_number'); setSortDir('asc');}} active={sortKey==='standard_number' && sortDir==='asc'}>رقم المعيار (تصاعدي)</Dropdown.Item>
                        ...
                        <div className="selection-bar d-flex flex-wrap align-items-center justify-content-between gap-2">
                          <div className="d-flex align-items-center gap-2">
                            ...
                            {pageAllSelected && selectedIds.size < sortedData.length && (
                              <button className="btn btn-link p-0" onClick={selectAllResults}>تحديد كل النتائج ({sortedData.length.toLocaleString('ar-SA')})</button>
                            )}
                          </div>
                          <div className="d-flex align-items-center gap-2">
                            <button className="btn btn-danger btn-sm" onClick={openBulkDelete}><i className="fas fa-trash-can ms-1" /> حذف المحدد</button>
                            <button className="btn btn-outline-secondary btn-sm" onClick={clearSelection}>مسح التحديد</button>
                          </div>
                        </div>
                      )}
                              ...
                              : hasPageData ? paginatedData.map((item, idx) => {
                                  ...
                                  return (
                                    <tr key={id}>
                                      ...
                                      <td><button className="btn btn-link p-0 text-primary" onClick={(e) => { e.preventDefault(); setModalItem(item); setShowModal(true); }}>إظهار</button></td>
                                      <td>{new Date(item.created_at).toLocaleDateString('ar-SA')}</td>
                                      {user?.role?.toLowerCase?.() !== 'user' && (
                                        <td>
                                          <button className="btn btn-link p-0 text-success" onClick={() => navigate(`/standards_edit/${item.standard_id}`)}><i className="fas fa-pen" /></button>
                                        </td>
                                      )}
                                    </tr>
                                  );
